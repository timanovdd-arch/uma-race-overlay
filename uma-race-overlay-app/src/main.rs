...
impl EguiOverlay for App {
    fn gui_run(
        ...
        if !self.placed {
            ...
            // ВАЖНО: окно НЕ должно точно совпадать с границами монитора.
            // Безрамочное + topmost + прозрачное окно, чья клиентская область
            // ТОЧНО равна разрешению монитора, заставляет Windows включить
            // Fullscreen Optimizations (FSO) — оверлей уходит в independent flip
            // в обход DWM, и прозрачный фон чернеет, как только окно в фокусе
            // (клик по таблице → passthrough=false → foreground → весь экран
            // тёмный; клик мимо → снова виден десктоп). Расширяем на 1px за
            // каждый край: совпадение ломается, FSO не срабатывает, лишний
            // пиксель уходит за пределы экрана и невидим. set_size/set_pos —
            // напрямую по сырому окну (физ. пиксели, единый базис).
            glfw_backend.window.set_pos(vx - 1, vy - 1);
            glfw_backend.window.set_size(vw + 2, vh + 2);
